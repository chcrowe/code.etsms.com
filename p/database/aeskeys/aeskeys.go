...
import (
	ecrypto "code.etsms.com/p/crypto"
	"code.etsms.com/p/database"
...
func GetMerchantAesCipher(dsn string, cid uint) (cipher.Block, []byte) {
	...
	//dsn := "server=192.168.3.113;user id=emwebadmin;password=?????;database=EMSecurity;"
...
func ExpandMerchantKeyset(b64EncryptedKey, b64EncryptedVector string) ([]byte, []byte) {
	...
	b64KeyEncryptingKey := "xHe8PGiDo7xmzwNBOEIhVukphahbhj6fG36bQdVwyvs="
	b64KeyEncryptingVector := "PhMQgzbGmr/+v/p3Bhj+sQ=="
...
func getKeys(db *sql.DB, cid uint) (list.NameValuePairMap, error) {
	...
	cmd := fmt.Sprintf("exec dbo._spAESKeys_Select %d ", cid)
